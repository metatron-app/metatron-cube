...
public class ConcatSequence<T> implements Sequence<T>
{
  ...
  )
  {
    ...
    return new Yielder<OutType>()
    {
      ...
      {
        final Yielder<OutType> next = yielder.next(initValue);
        if (next.isDone()) {
          // fixing NPE
          return makeYielder(yielderYielder.next(null), null, accumulator);
        }
        return wrapYielder(next, yielderYielder, accumulator);
      }
