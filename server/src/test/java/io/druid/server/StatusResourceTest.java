...
public class StatusResourceTest
{
  ...
  public void testPropertiesWithRestrictedConfigs()
  {
    ...
    Iterator iterator = Splitter.on(",").split(returnedProperties.get("druid.server.hiddenProperties")).iterator();
    while(iterator.hasNext()) {
      hiddenProperties.add(iterator.next().toString());
    }
    for (String s: hiddenProperties) {
      Assert.assertNull(returnedProperties.get(s));
    }
  }
}
