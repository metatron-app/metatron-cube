...
public class KafkaIndexTask extends AbstractTask implements ChatHandler
{
  ...
  )
  {
    ...
    this.parser = Preconditions.checkNotNull(
        dataSchema.getParser(jsonMapper, tuningConfig.isIgnoreInvalidRows()), "parser"
    );
    ...
    try (
      ...
      if (restoredMetadata == null) {
        ...
        Preconditions.checkState(sequences.get(0).startOffsets.entrySet().stream().allMatch(
            partitionOffsetEntry -> Long.compare(
                ...
                for (InputRow row : rows) {
                  if (row != null && withinMinMaxRecordTime(row)) {
                    ...
                    if (sequenceToUse == null) {
                      throw new ISE(
                          "cannot find any valid sequence for record with partition [%d] and offset [%d]. Current sequences: %s",
                          record.partition(),
                          record.offset(),
                          sequences
                      );
  ...
  {
    log.info("Starting up! runLegacy");
    startTime = DateTime.now();
    status = Status.STARTING;
    this.toolbox = toolbox;
  ...
  private static class SequenceMetadata
  {
    ...
    public void updateAssignments(Map<Integer, Long> nextPartitionOffset)
    {
      ...
      nextPartitionOffset.entrySet().forEach(partitionOffset -> {
        if (Long.compare(endOffsets.get(partitionOffset.getKey()), nextPartitionOffset.get(partitionOffset.getKey()))
            > 0) {
