...
public class SegmentsCostCache
{
  ...
  private static final Function<Bucket, Interval> BUCKET_TO_INTERVAL = new Function<Bucket, Interval>()
  {
    @Override
    public Interval apply(Bucket input)
    {
  ...
  private static final Comparator<DataSegment> SEGMENT_INTERVAL_COMPARATOR =
      Ordering.from(Comparators.intervalsByStartThenEnd()).onResultOf(
          new Function<DataSegment, Interval>()
          {
            @Override
            public Interval apply(DataSegment input)
            {
              return input.getInterval();
            }
          }
  ...
  private static final Comparator<Bucket> BUCKET_INTERVAL_COMPARATOR =
      Ordering.from(Comparators.intervalsByStartThenEnd()).onResultOf(BUCKET_TO_INTERVAL);
  ...
  SegmentsCostCache(ArrayList<Bucket> sortedBuckets)
  {
    ...
    this.intervals = Lists.newArrayList(Iterables.transform(sortedBuckets, BUCKET_TO_INTERVAL));
  ...
  public static class Builder
  {
    ...
    {
      Bucket.Builder builder = buckets.computeIfAbsent(
          getBucketInterval(segment),
          new java.util.function.Function<Interval, Bucket.Builder>()
          {
            @Override
            public Bucket.Builder apply(Interval input)
            {
              return new Bucket.Builder(input);
            }
          }
      );
    ...
    public Builder removeSegment(final DataSegment segment)
    {
      Interval interval = getBucketInterval(segment);
      buckets.computeIfPresent(
          interval, new BiFunction<Interval, Bucket.Builder, Bucket.Builder>()
          {
            @Override
            public Bucket.Builder apply( Interval interval, Bucket.Builder builder)
            {
              // If there are no move segments, returning null in computeIfPresent() removes the interval from the
              // buckets map
              return builder.removeSegment(segment).isEmpty() ? null : builder;
            }
          }
      );
      return this;
    ...
    {
      ArrayList<Bucket> sortedBuckets = Lists.newArrayList();
      for (Bucket.Builder builder : buckets.values()) {
        sortedBuckets.add(builder.build());
      }
      return new SegmentsCostCache(sortedBuckets);
    }
  ...
  static class Bucket
  {
    ...
    static class Builder
    {
      ...
      public Builder addSegment(DataSegment dataSegment)
      {
        ...
        for (SegmentAndSum v : segments.tailSet(segmentAndSum)) {
          v.leftSum += leftValue;
        }
        for (SegmentAndSum v : segments.headSet(segmentAndSum)) {
          v.rightSum += rightValue;
        }
      ...
      public Bucket build()
      {
        ...
        long bucketEndMillis = -1;
        for (DataSegment segment : segmentsList) {
          if (bucketEndMillis < 0 || segment.getInterval().getEndMillis() > bucketEndMillis) {
            bucketEndMillis = segment.getInterval().getEndMillis();
          }
        }
        if (bucketEndMillis < 0) {
          bucketEndMillis = interval.getEndMillis();
        }
