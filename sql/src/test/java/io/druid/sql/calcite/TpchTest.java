...
public class TpchTest extends TpchTestHelper
{
  ...
  public void tpch11() throws Exception
  {
    ...
    if (broadcastJoin) {
      ...
      if (semiJoin) {
        hook.verifyHooked(
            "0aIgu7G+/nruGDHy7LT+IQ==",
            "StreamQuery{dataSource='nation', filter=N_NAME=='GERMANY', columns=[N_NATIONKEY]}",
            "StreamQuery{dataSource='nation', filter=N_NAME=='GERMANY', columns=[N_NATIONKEY]}",
            "StreamQuery{dataSource='CommonJoin{queries=[GroupByQuery{dataSource='CommonJoin{queries=[StreamQuery{dataSource='partsupp', columns=[PS_AVAILQTY, PS_PARTKEY, PS_SUPPKEY, PS_SUPPLYCOST]}, StreamQuery{dataSource='supplier', filter=S_NATIONKEY=='7', columns=[S_NATIONKEY, S_SUPPKEY], $hash=true}], timeColumnName=__time}', dimensions=[DefaultDimensionSpec{dimension='PS_PARTKEY', outputName='d0'}], aggregatorSpecs=[GenericSumAggregatorFactory{name='a0', fieldExpression='(PS_SUPPLYCOST * PS_AVAILQTY)', inputType='double'}], outputColumns=[d0, a0]}, TimeseriesQuery{dataSource='CommonJoin{queries=[StreamQuery{dataSource='partsupp', columns=[PS_AVAILQTY, PS_SUPPKEY, PS_SUPPLYCOST]}, StreamQuery{dataSource='supplier', filter=S_NATIONKEY=='7', columns=[S_NATIONKEY, S_SUPPKEY], $hash=true}], timeColumnName=__time}', aggregatorSpecs=[GenericSumAggregatorFactory{name='a0', fieldExpression='(PS_SUPPLYCOST * PS_AVAILQTY)', inputType='double'}], outputColumns=[a0], $hash=true}], timeColumnName=__time}', filter=MathExprFilter{expression='(a0 > (a00 * 0.0001B))'}, columns=[d0, a0], orderingSpecs=[OrderByColumnSpec{dimension='a0', direction=descending}]}",
            ...
            "TimeseriesQuery{dataSource='CommonJoin{queries=[StreamQuery{dataSource='partsupp', columns=[PS_AVAILQTY, PS_SUPPKEY, PS_SUPPLYCOST]}, StreamQuery{dataSource='supplier', filter=S_NATIONKEY=='7', columns=[S_NATIONKEY, S_SUPPKEY], $hash=true}], timeColumnName=__time}', aggregatorSpecs=[GenericSumAggregatorFactory{name='a0', fieldExpression='(PS_SUPPLYCOST * PS_AVAILQTY)', inputType='double'}], outputColumns=[a0], $hash=true}",
      ...
      if (broadcastJoin) {
        ...
        if (bloomFilter) {
          ...
          hook.verifyHooked(
              ...
              "StreamQuery{dataSource='nation', filter=N_NAME=='UNITED STATES', columns=[N_NATIONKEY]}",
              "StreamQuery{dataSource='lineitem', filter=(InDimFilter{dimension='L_ORDERKEY', values=[10018, 10021, 10022, 10048, 10274, 10305, 10434, 10437, 10438, 10528, ..535 more]} && MathExprFilter{expression='(L_RECEIPTDATE > L_COMMITDATE)'}), columns=[L_SUPPKEY]}",
              "GroupByQuery{dataSource='CommonJoin{queries=[StreamQuery{dataSource='lineitem', filter=(InDimFilter{dimension='L_ORDERKEY', values=[10018, 10021, 10022, 10048, 10274, 10305, 10434, 10437, 10438, 10528, ..535 more]} && MathExprFilter{expression='(L_RECEIPTDATE > L_COMMITDATE)'}), columns=[L_SUPPKEY]}, StreamQuery{dataSource='supplier', filter=S_NATIONKEY=='24', columns=[S_NAME, S_SUPPKEY], $hash=true}], timeColumnName=__time}', dimensions=[DefaultDimensionSpec{dimension='S_NAME', outputName='d0'}], aggregatorSpecs=[CountAggregatorFactory{name='a0'}], limitSpec=LimitSpec{columns=[OrderByColumnSpec{dimension='a0', direction=descending}, OrderByColumnSpec{dimension='d0', direction=ascending}], limit=100}, outputColumns=[d0, a0]}",
              "StreamQuery{dataSource='lineitem', filter=(InDimFilter{dimension='L_ORDERKEY', values=[10018, 10021, 10022, 10048, 10274, 10305, 10434, 10437, 10438, 10528, ..535 more]} && MathExprFilter{expression='(L_RECEIPTDATE > L_COMMITDATE)'}), columns=[L_SUPPKEY]}",
              "StreamQuery{dataSource='supplier', filter=S_NATIONKEY=='24', columns=[S_NAME, S_SUPPKEY], $hash=true}"
          );
        }
