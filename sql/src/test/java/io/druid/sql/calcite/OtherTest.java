...
public class OtherTest extends CalciteQueryTestHelper
{
  ...
  public void testX() throws Exception
  {
    ...
    hook.verifyHooked(
        "TEgTAzhwcWhECF4cnf1iFw==",
        "GroupByQuery{dataSource='b', querySegmentSpec=MultipleIntervalSegmentSpec{intervals=[2023-02-02T23:16:37.000Z/2023-02-05T02:16:37.000Z]}, dimensions=[DefaultDimensionSpec{dimension='d0:v', outputName='d0'}, DefaultDimensionSpec{dimension='modelType', outputName='d1'}], filter=assetId=='CVD_8G_PM_CH', virtualColumns=[ExprVirtualColumn{expression='TIMESTAMP_FORMAT(timestamp_ceil(__time,'PT10M','','UTC'),'yyyy-MM-dd\\u0027T\\u0027HH:mm:ss.SSS\\u0027Z\\u0027','UTC')', outputName='d0:v'}], aggregatorSpecs=[GenericSumAggregatorFactory{name='a0:sum', fieldName='score', inputType='double'}, CountAggregatorFactory{name='a0:count', fieldName='score'}, GenericSumAggregatorFactory{name='a1:sum', fieldName='smoothingScore', inputType='double'}, CountAggregatorFactory{name='a1:count', fieldName='smoothingScore'}], postAggregatorSpecs=[ArithmeticPostAggregator{name='a0', fnName='quotient', fields=[FieldAccessPostAggregator{fieldName='a0:sum'}, FieldAccessPostAggregator{fieldName='a0:count'}], op=QUOTIENT}, ArithmeticPostAggregator{name='a1', fnName='quotient', fields=[FieldAccessPostAggregator{fieldName='a1:sum'}, FieldAccessPostAggregator{fieldName='a1:count'}], op=QUOTIENT}], outputColumns=[d0, d1, a0, a1]}"
    );
