    ...
    if (initialized.compareAndSet(true, true)) {
      try {
        ...
        CompletableFuture<CachingCostBalancerStrategy> future = CompletableFuture.supplyAsync(
            new Supplier<CachingCostBalancerStrategy>()
            {
              @Override
              public CachingCostBalancerStrategy get()
              {
                return new CachingCostBalancerStrategy(clusterCostCacheBuilder.build(), exec);
              }
            },
            executor
        );
