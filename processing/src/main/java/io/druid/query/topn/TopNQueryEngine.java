...
public class TopNQueryEngine
{
  ...
  public Sequence<Result<TopNResultValue>> query(final TopNQuery query, final Segment segment, final SessionCache cache)
  {
    ...
    return QueryRunnerHelper.makeCursorBasedQuery(adapter, query, cache, getMapFn(query, adapter));
  }
  ...
  private Function<Cursor, Result<TopNResultValue>> getMapFn(TopNQuery query, final StorageAdapter adapter)
  {
    ...
    log.debug("with [%s].. using algorithm [%s]", selector, topNAlgorithm.getClass().getSimpleName());
