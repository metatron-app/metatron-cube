...
/**
 * The overlord's task/supervisor wiring, extracted so it can be loaded either by the standalone
 * {@link CliOverlord} or embedded into the coordinator process (coordinator-as-overlord).
 *
 * This binds only the indexing-service-specific things; host-specific bindings that the coordinator
 * already provides (service type/port, AuditManager, RedirectFilter/RedirectInfo, the
 * JettyServerInitializer and the Jetty Server lifecycle) are intentionally left to the host runner.
 *
 * Companion modules {@code IndexingServiceFirehoseModule} and {@code IndexingServiceTaskLogsModule}
 * must be added alongside this one (both CliOverlord and the embedded path do so).
 */
public class OverlordModule implements Module
{
  ...
  private void configureAutoscale(Binder binder)
  {
    ...
    final MapBinder<String, ResourceManagementStrategy> biddy =
        PolyBind.optionBinder(binder, Key.get(ResourceManagementStrategy.class));
