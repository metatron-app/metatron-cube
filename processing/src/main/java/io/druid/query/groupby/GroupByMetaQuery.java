...
public class GroupByMetaQuery extends BaseQuery<Row> implements Query.RewritingQuery<Row>, Query.WrappingQuery<Row>
{
  ...
  public Query<Row> rewriteQuery(QuerySegmentWalker segmentWalker)
  {
    try {
      Query<Row> rewritten = query.toCardinalityEstimator(segmentWalker);
      if (rewritten instanceof RewritingQuery) {
        rewritten = ((RewritingQuery)rewritten).rewriteQuery(segmentWalker);
      }
      return rewritten;
    }
    catch (Exception e) {
