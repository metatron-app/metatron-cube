...
/**
 */
public abstract class LongMaxAggregator implements Aggregator.FromMutableLong, Aggregator.LongStreaming
{
    ...
    return new LongMaxAggregator()
    {
      ...
      public MutableLong aggregate(final MutableLong current)
      {
        if (selector.getLong(handover)) {
          ...
          current.setValue(Math.max(current.longValue(), handover.longValue()));
        }
        return current;
      }
    };
      ...
      public void aggregate(IntIterator iterator, MutableLong[] vector, Int2IntFunction offset)
      {
        selector.consume(iterator, (i, x) -> {
          ...
          if (vector[ix] == null) {
            ...
            vector[ix].setValue(Math.max(vector[ix].longValue(), x));
          }
        });
      }
