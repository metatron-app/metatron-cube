...
def get_open_pull_requests(git_url_str, authors, issues):
    ...
    for pr in pull_json:
        ...
        if creator in authors:
            pr_detail = rest_get(pr_url).json()
...
def apply_patches(git_dir, repo, patch_dir, issue_nums, info, do_commit=False):
...
def push_tag_and_branch(repo):
    # push branch
    origin = repo.remotes.origin
    cur_brname = str(repo.active_branch)

    # check current branche name, if it does not start with build_, do NOT push it
    if cur_brname.startswith('build_'):
...
if __name__ == "__main__":
    ...
    if 5 in steps:
        ...
        apply_patches(git_dir, repo, patch_dir, issues, info, do_commit=True)
