...
/**
 * from org.apache.calcite.rel.rules.AggregateMergeRule.. see ensureTypes()
 *
...
public class AggregateMergeRule extends RelOptRule {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    for (AggregateCall topCall : topAgg.getAggCallList()) {
      ...
      AggregateCall finalCall = ensureTypes(
          topCall, splitter.merge(topCall, bottomCall), topAgg.getCluster().getTypeFactory()
      );
  ...
  private AggregateCall ensureTypes(AggregateCall topCall, AggregateCall finalCall, RelDataTypeFactory typeFactory)
  {
    if (finalCall == null) {
      return null;
    }
    RelDataType original = topCall.getType();
    RelDataType merged = finalCall.getType();
    if (original.equals(merged)) {
      return finalCall;
    }
    if (original.isNullable() && !merged.isNullable()) {
      // I don't care calling it nullable
      merged = typeFactory.createTypeWithNullability(merged, true);
      if (original.equals(merged)) {
        return AggregateCall.create(
            finalCall.getAggregation(),
            finalCall.isDistinct(), finalCall.isApproximate(), finalCall.ignoreNulls(),
            finalCall.getArgList(), finalCall.filterArg, finalCall.getCollation(), merged, finalCall.getName()
        );
      }
    }
    return null;
