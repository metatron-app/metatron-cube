...
public class TpchTest extends TpchTestHelper
{
  ...
  public void tpch4() throws Exception
  {
    ...
    if (semiJoin) {
      hook.verifyHooked(
          ...
          "GroupByQuery{dataSource='StreamQuery{dataSource='orders', filter=(BoundDimFilter{1996-05-01 <= O_ORDERDATE < 1996-08-01} && !(O_ORDERKEY==NULL) && CompressedInFilter{dimension='O_ORDERKEY', sourceLen=6953, destLen=21978}), columns=[O_ORDERPRIORITY]}', dimensions=[DefaultDimensionSpec{dimension='O_ORDERPRIORITY', outputName='d0'}], aggregatorSpecs=[CountAggregatorFactory{name='a0'}], limitSpec=LimitSpec{columns=[OrderByColumnSpec{dimension='d0', direction=ascending}], limit=-1}, outputColumns=[d0, a0]}",
          "StreamQuery{dataSource='orders', filter=(BoundDimFilter{1996-05-01 <= O_ORDERDATE < 1996-08-01} && !(O_ORDERKEY==NULL) && CompressedInFilter{dimension='O_ORDERKEY', sourceLen=6953, destLen=21978}), columns=[O_ORDERPRIORITY]}"
      );
    } else {
      if (bloomFilter) {
        hook.verifyHooked(
    ...
    if (semiJoin) {
      if (broadcastJoin) {
        hook.verifyHooked(
            ...
            "StreamQuery{dataSource='lineitem', filter=(BoundDimFilter{1995-01-01 <= L_SHIPDATE <= 1996-12-31} && CompressedInFilter{dimension='L_ORDERKEY', sourceLen=2679, destLen=23732}), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_ORDERKEY, L_SUPPKEY, v0], virtualColumns=[ExprVirtualColumn{expression='YEAR(L_SHIPDATE)', outputName='v0'}], localPostProcessing=BroadcastJoinProcessor{element=JoinElement{joinType=INNER, leftAlias=supplier, leftJoinColumns=[S_SUPPKEY], rightAlias=lineitem, rightJoinColumns=[L_SUPPKEY]}, hashLeft=true, hashSignature={S_NATIONKEY:string, S_SUPPKEY:string}}}",
        ...
        hook.verifyHooked(
            ...
            "StreamQuery{dataSource='lineitem', filter=(BoundDimFilter{1995-01-01 <= L_SHIPDATE <= 1996-12-31} && InDimFilter{dimension='L_SUPPKEY', values=[1, 10, 11, 12, 13, 14, 15, 16, 17, 18, ..40 more]} && CompressedInFilter{dimension='L_ORDERKEY', sourceLen=2679, destLen=23732}), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_ORDERKEY, L_SUPPKEY, v0], virtualColumns=[ExprVirtualColumn{expression='YEAR(L_SHIPDATE)', outputName='v0'}]}",
            "StreamQuery{dataSource='customer', filter=InDimFilter{dimension='C_NATIONKEY', values=[14, 17]}, columns=[C_CUSTKEY, C_NATIONKEY], $hash=true}",
            "StreamQuery{dataSource='nation', filter=InDimFilter{dimension='N_NAME', values=[KENYA, PERU]}, columns=[N_NAME, N_NATIONKEY], $hash=true}"
        );
      }
    ...
    if (semiJoin) {
      ...
      if (bloomFilter) {
        hook.verifyHooked(
            ...
            "StreamQuery{dataSource='lineitem', filter=BoundDimFilter{1995-08-01 <= L_SHIPDATE < 1995-09-01}, columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY], $hash=true}",
            "StreamQuery{dataSource='part', filter=BloomFilter{fields=[DefaultDimensionSpec{dimension='P_PARTKEY', outputName='P_PARTKEY'}], groupingSets=Noop}, columns=[P_PARTKEY, v0], virtualColumns=[ExprVirtualColumn{expression='like(P_TYPE,'PROMO%')', outputName='v0'}]}"
        );
      } else {
        hook.verifyHooked(
  ...
  public static final String TPCH15_EXPLAIN_JR =
      "DruidOuterQueryRel(sort=[$0:ASC])\n"
      + "  DruidJoinRel(joinType=[INNER], leftKeys=[0], rightKeys=[0], outputColumns=[4, 2, 1, 3, 0])\n"
      + "    DruidJoinRel(joinType=[INNER], leftKeys=[0], rightKeys=[3], outputColumns=[1, 2, 3, 4, 5])\n"
      + "      DruidQueryRel(table=[druid.lineitem], scanFilter=[SEARCH($11, Sarg[['1996-01-01'..'1996-04-01')]:CHAR(10))], scanProject=[$14, *($3, -(1, $2))], group=[{0}], TOTAL_REVENUE=[SUM($1)])\n"
      + "      DruidQueryRel(table=[druid.supplier], scanProject=[$1, $3, $5, $6])\n"
      + "    DruidOuterQueryRel(EXPR$0=[MAX($0)])\n"
      + "      DruidQueryRel(table=[druid.lineitem], scanFilter=[SEARCH($11, Sarg[['1996-01-01'..'1996-04-01')]:CHAR(10))], scanProject=[$14, *($3, -(1, $2))], group=[{0}], TOTAL_REVENUE=[SUM($1)], aggregateProject=[$1])\n";
  ...
  public static final String TPCH19_EXPLAIN =
      ...
      + "    DruidQueryRel(table=[druid.lineitem], scanFilter=[AND(=($12, 'NONE'), SEARCH($13, Sarg['AIR':VARCHAR(7), 'AIR REG':VARCHAR(7)]:VARCHAR(7)), IS NOT NULL($8))], scanProject=[$2, $3, $7, $8])\n"
  ...
  public void tpch19() throws Exception
  {
    ...
    if (broadcastJoin) {
      hook.verifyHooked(
          ...
          "TimeseriesQuery{dataSource='StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL)), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY], localPostProcessing=BroadcastJoinProcessor{element=JoinElement{joinType=INNER, leftAlias=lineitem, leftJoinColumns=[L_PARTKEY], rightAlias=part, rightJoinColumns=[P_PARTKEY]}, hashLeft=false, hashSignature={P_BRAND:string, P_CONTAINER:string, P_PARTKEY:string, P_SIZE:long}, applyFilter=true}}', filter=((P_BRAND=='Brand#32' && InDimFilter{dimension='P_CONTAINER', values=[SM BOX, SM CASE, SM PACK, SM PKG]} && BoundDimFilter{7 <= L_QUANTITY <= 17(numeric)} && BoundDimFilter{1 <= P_SIZE <= 5(numeric)}) || (P_BRAND=='Brand#35' && InDimFilter{dimension='P_CONTAINER', values=[MED BAG, MED BOX, MED PACK, MED PKG]} && BoundDimFilter{15 <= L_QUANTITY <= 25(numeric)} && BoundDimFilter{1 <= P_SIZE <= 10(numeric)}) || (P_BRAND=='Brand#24' && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG]} && BoundDimFilter{26 <= L_QUANTITY <= 36(numeric)} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)})), aggregatorSpecs=[GenericSumAggregatorFactory{name='a0', fieldExpression='(L_EXTENDEDPRICE * (1 - L_DISCOUNT))', inputType='double'}], outputColumns=[a0]}",
          "StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL)), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY], localPostProcessing=BroadcastJoinProcessor{element=JoinElement{joinType=INNER, leftAlias=lineitem, leftJoinColumns=[L_PARTKEY], rightAlias=part, rightJoinColumns=[P_PARTKEY]}, hashLeft=false, hashSignature={P_BRAND:string, P_CONTAINER:string, P_PARTKEY:string, P_SIZE:long}, applyFilter=true}}"
      );
    } else if (semiJoin) {
      hook.verifyHooked(
          "VkhiP+AdirT+paQfSZvztw==",
          "StreamQuery{dataSource='part', filter=(InDimFilter{dimension='P_BRAND', values=[Brand#24, Brand#32, Brand#35]} && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG, MED BAG, MED BOX, MED PACK, MED PKG, SM BOX, SM CASE, ..2 more]} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)}), columns=[P_BRAND, P_CONTAINER, P_PARTKEY, P_SIZE], $hash=true}",
          "TimeseriesQuery{dataSource='CommonJoin{queries=[StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL) && InDimFilter{dimension='L_PARTKEY', values=[124, 139, 149, 16, 208, 395, 408, 5, 6, 620, ..10 more]}), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY]}, MaterializedQuery{dataSource=[part]}], timeColumnName=__time}', filter=((P_BRAND=='Brand#32' && InDimFilter{dimension='P_CONTAINER', values=[SM BOX, SM CASE, SM PACK, SM PKG]} && BoundDimFilter{7 <= L_QUANTITY <= 17(numeric)} && BoundDimFilter{1 <= P_SIZE <= 5(numeric)}) || (P_BRAND=='Brand#35' && InDimFilter{dimension='P_CONTAINER', values=[MED BAG, MED BOX, MED PACK, MED PKG]} && BoundDimFilter{15 <= L_QUANTITY <= 25(numeric)} && BoundDimFilter{1 <= P_SIZE <= 10(numeric)}) || (P_BRAND=='Brand#24' && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG]} && BoundDimFilter{26 <= L_QUANTITY <= 36(numeric)} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)})), aggregatorSpecs=[GenericSumAggregatorFactory{name='a0', fieldExpression='(L_EXTENDEDPRICE * (1 - L_DISCOUNT))', inputType='double'}], outputColumns=[a0]}",
          "StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL) && InDimFilter{dimension='L_PARTKEY', values=[124, 139, 149, 16, 208, 395, 408, 5, 6, 620, ..10 more]}), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY]}"
      );
    } else {
      if (bloomFilter) {
        hook.verifyHooked(
            "pL6KAKpXzCi00JwoaFOx9g==",
            "TimeseriesQuery{dataSource='CommonJoin{queries=[StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL) && BloomDimFilter.Factory{bloomSource=$view:part[P_PARTKEY]((InDimFilter{dimension='P_BRAND', values=[Brand#24, Brand#32, Brand#35]} && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG, MED BAG, MED BOX, MED PACK, MED PKG, SM BOX, SM CASE, ..2 more]} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)})), fields=[DefaultDimensionSpec{dimension='L_PARTKEY', outputName='L_PARTKEY'}], groupingSets=Noop, maxNumEntries=20}), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY]}, StreamQuery{dataSource='part', filter=(InDimFilter{dimension='P_BRAND', values=[Brand#24, Brand#32, Brand#35]} && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG, MED BAG, MED BOX, MED PACK, MED PKG, SM BOX, SM CASE, ..2 more]} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)}), columns=[P_BRAND, P_CONTAINER, P_PARTKEY, P_SIZE], $hash=true}], timeColumnName=__time}', filter=((P_BRAND=='Brand#32' && InDimFilter{dimension='P_CONTAINER', values=[SM BOX, SM CASE, SM PACK, SM PKG]} && BoundDimFilter{7 <= L_QUANTITY <= 17(numeric)} && BoundDimFilter{1 <= P_SIZE <= 5(numeric)}) || (P_BRAND=='Brand#35' && InDimFilter{dimension='P_CONTAINER', values=[MED BAG, MED BOX, MED PACK, MED PKG]} && BoundDimFilter{15 <= L_QUANTITY <= 25(numeric)} && BoundDimFilter{1 <= P_SIZE <= 10(numeric)}) || (P_BRAND=='Brand#24' && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG]} && BoundDimFilter{26 <= L_QUANTITY <= 36(numeric)} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)})), aggregatorSpecs=[GenericSumAggregatorFactory{name='a0', fieldExpression='(L_EXTENDEDPRICE * (1 - L_DISCOUNT))', inputType='double'}], outputColumns=[a0]}",
            ...
            "StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL) && BloomFilter{fields=[DefaultDimensionSpec{dimension='L_PARTKEY', outputName='L_PARTKEY'}], groupingSets=Noop}), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY]}",
            "StreamQuery{dataSource='part', filter=(InDimFilter{dimension='P_BRAND', values=[Brand#24, Brand#32, Brand#35]} && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG, MED BAG, MED BOX, MED PACK, MED PKG, SM BOX, SM CASE, ..2 more]} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)}), columns=[P_BRAND, P_CONTAINER, P_PARTKEY, P_SIZE], $hash=true}"
        );
      } else {
        hook.verifyHooked(
            "mPRA3yAXvMcINL6SZtwdIQ==",
            "TimeseriesQuery{dataSource='CommonJoin{queries=[StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL)), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY]}, StreamQuery{dataSource='part', filter=(InDimFilter{dimension='P_BRAND', values=[Brand#24, Brand#32, Brand#35]} && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG, MED BAG, MED BOX, MED PACK, MED PKG, SM BOX, SM CASE, ..2 more]} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)}), columns=[P_BRAND, P_CONTAINER, P_PARTKEY, P_SIZE], $hash=true}], timeColumnName=__time}', filter=((P_BRAND=='Brand#32' && InDimFilter{dimension='P_CONTAINER', values=[SM BOX, SM CASE, SM PACK, SM PKG]} && BoundDimFilter{7 <= L_QUANTITY <= 17(numeric)} && BoundDimFilter{1 <= P_SIZE <= 5(numeric)}) || (P_BRAND=='Brand#35' && InDimFilter{dimension='P_CONTAINER', values=[MED BAG, MED BOX, MED PACK, MED PKG]} && BoundDimFilter{15 <= L_QUANTITY <= 25(numeric)} && BoundDimFilter{1 <= P_SIZE <= 10(numeric)}) || (P_BRAND=='Brand#24' && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG]} && BoundDimFilter{26 <= L_QUANTITY <= 36(numeric)} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)})), aggregatorSpecs=[GenericSumAggregatorFactory{name='a0', fieldExpression='(L_EXTENDEDPRICE * (1 - L_DISCOUNT))', inputType='double'}], outputColumns=[a0]}",
            "StreamQuery{dataSource='lineitem', filter=(L_SHIPINSTRUCT=='NONE' && InDimFilter{dimension='L_SHIPMODE', values=[AIR, AIR REG]} && !(L_QUANTITY==NULL)), columns=[L_DISCOUNT, L_EXTENDEDPRICE, L_PARTKEY, L_QUANTITY]}",
            "StreamQuery{dataSource='part', filter=(InDimFilter{dimension='P_BRAND', values=[Brand#24, Brand#32, Brand#35]} && InDimFilter{dimension='P_CONTAINER', values=[LG BOX, LG CASE, LG PACK, LG PKG, MED BAG, MED BOX, MED PACK, MED PKG, SM BOX, SM CASE, ..2 more]} && BoundDimFilter{1 <= P_SIZE <= 15(numeric)}), columns=[P_BRAND, P_CONTAINER, P_PARTKEY, P_SIZE], $hash=true}"
        );
      }
    }
  ...
  public void tpch21() throws Exception
  {
    ...
    if (semiJoin) {
      hook.verifyHooked(
          ...
          "StreamQuery{dataSource='lineitem', filter=(MathExprFilter{expression='(L_RECEIPTDATE > L_COMMITDATE)'} && CompressedInFilter{dimension='L_ORDERKEY', sourceLen=7303, destLen=12308} && CompressedInFilter{dimension='L_ORDERKEY', sourceLen=7744, destLen=20622} && CompressedInFilter{dimension='L_ORDERKEY', sourceLen=4394, destLen=6372}), columns=[L_SUPPKEY]}",
  ...
  public static final String TPCH22_EXPLAIN_JR =
      "DruidOuterQueryRel(scanFilter=[AND(>($1, $2), IS NULL($3))], scanProject=[SUBSTR($0, 1, 2), $1], group=[{0}], NUMCUST=[COUNT()], TOTACCTBAL=[SUM($1)], sort=[$0:ASC])\n"
      + "  DruidJoinRel(joinType=[INNER], outputColumns=[4, 2, 5, 1])\n"
