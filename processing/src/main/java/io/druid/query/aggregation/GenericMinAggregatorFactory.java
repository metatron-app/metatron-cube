...
/**
 */
public class GenericMinAggregatorFactory extends GenericAggregatorFactory implements AggregatorFactory.Vectorizable
{
  ...
  public BinaryFn.Identical combiner()
  {
    switch (inputType.type()) {
      case FLOAT:
        return (param1, param2) -> ((Number) param1).floatValue() > ((Number) param2).floatValue() ? param2 : param1;
      case DOUBLE:
        return (param1, param2) -> ((Number) param1).doubleValue() > ((Number) param2).doubleValue() ? param2 : param1;
      case LONG:
        return (param1, param2) -> ((Number) param1).longValue() > ((Number) param2).longValue() ? param2 : param1;
    }
  ...
  public Aggregator.Vectorized create(ColumnSelectorFactory factory)
  {
    switch (inputType.type()) {
      case LONG:
        return LongMinAggregator.vectorize((LongColumnSelector.Scannable) factory.makeLongColumnSelector(fieldName));
      case FLOAT:
      case DOUBLE:
        return DoubleMinAggregator.vectorize((DoubleColumnSelector.Scannable) factory.makeDoubleColumnSelector(fieldName));
      default:
