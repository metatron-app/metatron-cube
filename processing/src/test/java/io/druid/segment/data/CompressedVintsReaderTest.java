...
public class CompressedVintsReaderTest extends CompressionStrategyTest
{
  ...
  public CompressedVintsReaderTest(CompressedObjectStrategy.CompressionStrategy compressionStrategy, ByteOrder byteOrder)
  {
  ...
  private CompressedVintsReader supplier;
  private int[] vals;
  ...
  private void setupSimple(final int chunkSize)
  {
    ...
    supplier = CompressedVintsReader.fromList(
        Ints.asList(vals),
        Ints.max(vals),
        chunkSize,
  ...
  private void makeWithSerde(final int chunkSize) throws IOException
  {
    ...
    final CompressedVintsReader theSupplier = CompressedVintsReader.fromList(
    ...
    supplier = CompressedVintsReader.from(ByteBuffer.wrap(bytes), byteOrder);
    indexed = supplier.get();
  }
  ...
  public void testLargeChunks() throws Exception
  {
    for (int maxValue : MAX_VALUES) {
      final int maxChunkSize = CompressedVintsReader.maxIntsInBufferForValue(maxValue);
  ...
  {
    Assert.assertEquals(CompressedPools.BUFFER_SIZE, CompressedVintsReader.maxIntsInBufferForBytes(1));
    Assert.assertEquals(CompressedPools.BUFFER_SIZE / 2, CompressedVintsReader.maxIntsInBufferForBytes(2));
    Assert.assertEquals(CompressedPools.BUFFER_SIZE / 4, CompressedVintsReader.maxIntsInBufferForBytes(4));

    Assert.assertEquals(CompressedPools.BUFFER_SIZE, 0x10000); // nearest power of 2 is 2^14
    Assert.assertEquals(1 << 14, CompressedVintsReader.maxIntsInBufferForBytes(3));
  }
