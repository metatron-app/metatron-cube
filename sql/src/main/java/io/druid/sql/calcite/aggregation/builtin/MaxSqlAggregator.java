...
public class MaxSqlAggregator implements SqlAggregator
{
  ...
  public boolean register(Aggregations aggregations, DimFilter predicate, AggregateCall call, String outputName)
  {
    ...
    if (outputType.isNumeric() || outputType.isArray()) {
      factory = GenericMaxAggregatorFactory.of(outputName, inputName, outputType);
    } else if (expression.isDirectColumnAccess()) {
      factory = RelayAggregatorFactory.max(outputName, expression.getDirectColumn(), outputType.typeName());
    } else {
      throw new UnsupportedOperationException("Not supports max on non-numeric expression");
    }
